...
| Charades-STA 视频事件 | 1500 | 0.390 | 0.363 | 0.811 | 0.826 | 0.543 | **0.859** | 0.021 |
...
**延迟**（一张空闲 A100、线上服务路径、768 token 图片预算、同一张图的多个问题合成一次请求，`bench/speed_bench.py` 跑 12 次取中位数）：OmniJev-0.8B 单问 160 ms、十二问 186 ms；OmniJev-2B 单问 179 ms、十二问 216 ms。OmniJev-4B 换到 Qwen3.5 底座后正在重测；通过线上接口，同一张图问三个问题约 240 ms。

4B 走单次前向的打包路径；0.8B 和 2B（Qwen3.5 底座，混合注意力）走前缀分支路径，所以小模型单次请求并不更快。
